...
app.post("/email-verificacao", (req,res) => {
    ...
    const {email, token, nome} = req.body;

    transport.sendMail({
        from:`Mente Financeira <${process.env.USER}>`,
        to:`${email}`,
        subject: 'Equipe Mente Financeira💵',
        html:`<h2>Olá, ${nome}, aqui é a equipe do <strong style="color:green;">Mente Financeira</strong>💰💵</h2> <h4>Para melhorar a segurança e sua experiência, segue o link para validar seu email:</h4>`+
                `<a href="http://localhost:5500/pages/verificar-email.html?token=${token}">http://localhost:5500/pages/verificar-email.html?token=${token}</a>`+
